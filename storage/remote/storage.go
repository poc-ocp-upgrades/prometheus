...
func (s *Storage) ApplyConfig(conf *config.Config) error {
	...
	for i, rwConf := range conf.RemoteWriteConfigs {
		c, err := NewClient(i, &ClientConfig{URL: rwConf.URL, Timeout: rwConf.RemoteTimeout, HTTPClientConfig: rwConf.HTTPClientConfig})
		if err != nil {
			return err
		}
		newQueues = append(newQueues, NewQueueManager(s.logger, rwConf.QueueConfig, conf.GlobalConfig.ExternalLabels, rwConf.WriteRelabelConfigs, c, s.flushDeadline))
	}
	...
	for i, rrConf := range conf.RemoteReadConfigs {
		c, err := NewClient(i, &ClientConfig{URL: rrConf.URL, Timeout: rrConf.RemoteTimeout, HTTPClientConfig: rrConf.HTTPClientConfig})
		if err != nil {
			return err
		}
		q := QueryableClient(c)
...
func labelsToEqualityMatchers(ls model.LabelSet) []*labels.Matcher {
	...
	for k, v := range ls {
		ms = append(ms, &labels.Matcher{Type: labels.MatchEqual, Name: string(k), Value: string(v)})
	}
	return ms
}
