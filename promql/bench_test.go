...
func BenchmarkRangeQuery(b *testing.B) {
	...
	cases := []benchCase{{expr: "a_X"}, {expr: "rate(a_X[1m])"}, {expr: "rate(a_X[1m])", steps: 10000}, {expr: "holt_winters(a_X[1d], 0.3, 0.3)"}, {expr: "changes(a_X[1d])"}, {expr: "rate(a_X[1d])"}, {expr: "-a_X"}, {expr: "a_X - b_X"}, {expr: "a_X - b_X", steps: 10000}, {expr: "a_X and b_X{l=~'.*[0-4]$'}"}, {expr: "a_X or b_X{l=~'.*[0-4]$'}"}, {expr: "a_X unless b_X{l=~'.*[0-4]$'}"}, {expr: "abs(a_X)"}, {expr: "label_replace(a_X, 'l2', '$1', 'l', '(.*)')"}, {expr: "label_join(a_X, 'l2', '-', 'l', 'l')"}, {expr: "sum(a_X)"}, {expr: "sum without (l)(h_X)"}, {expr: "sum without (le)(h_X)"}, {expr: "sum by (l)(h_X)"}, {expr: "sum by (le)(h_X)"}, {expr: "rate(a_X[1m]) + rate(b_X[1m])"}, {expr: "sum without (l)(rate(a_X[1m]))"}, {expr: "sum without (l)(rate(a_X[1m])) / sum without (l)(rate(b_X[1m]))"}, {expr: "histogram_quantile(0.9, rate(h_X[5m]))"}}
	tmp := []benchCase{}
	for _, c := range cases {
	...
	for _, c := range cases {
		...
		b.Run(name, func(b *testing.B) {
			b.ReportAllocs()
			for i := 0; i < b.N; i++ {
				qry, err := engine.NewRangeQuery(storage, c.expr, time.Unix(int64((numIntervals-c.steps)*10), 0), time.Unix(int64(numIntervals*10), 0), time.Second*10)
				if err != nil {
					b.Fatal(err)
				}
