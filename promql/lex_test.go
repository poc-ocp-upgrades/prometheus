	...
	for _, typ := range tests {
		t.Run(typ.name, func(t *testing.T) {
			for i, test := range typ.tests {
				l := &lexer{input: test.input, items: make(chan item), seriesDesc: test.seriesDesc}
				go l.run()
				out := []item{}
