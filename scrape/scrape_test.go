...
func TestDroppedTargetsList(t *testing.T) {
	...
	var (
		app			= &nopAppendable{}
		cfg			= &config.ScrapeConfig{JobName: "dropMe", ScrapeInterval: model.Duration(1), RelabelConfigs: []*relabel.Config{{Action: relabel.Drop, Regex: relabel.MustNewRegexp("dropMe"), SourceLabels: model.LabelNames{"job"}}}}
		tgs			= []*targetgroup.Group{{Targets: []model.LabelSet{{model.AddressLabel: "127.0.0.1:9090"}}}}
		sp			= newScrapePool(cfg, app, nil)
...
func TestDiscoveredLabelsUpdate(t *testing.T) {
	...
	t1 := &Target{discoveredLabels: labels.Labels{labels.Label{Name: "label", Value: "name"}}}
	sp.activeTargets[t1.hash()] = t1
	t2 := &Target{discoveredLabels: labels.Labels{labels.Label{Name: "labelNew", Value: "nameNew"}}}
	sp.sync([]*Target{t2})
...
func TestScrapePoolStop(t *testing.T) {
	...
	for i := 0; i < numTargets; i++ {
		t := &Target{labels: labels.FromStrings(model.AddressLabel, fmt.Sprintf("example.com:%d", i))}
		l := &testLoop{}
		l.stopFunc = func() {
...
func TestScrapePoolReload(t *testing.T) {
	...
	numTargets := 20
	stopped := map[uint64]bool{}
	reloadCfg := &config.ScrapeConfig{ScrapeInterval: model.Duration(3 * time.Second), ScrapeTimeout: model.Duration(2 * time.Second)}
	...
	sp := &scrapePool{appendable: &nopAppendable{}, activeTargets: map[uint64]*Target{}, loops: map[uint64]loop{}, newLoop: newLoop, logger: nil}
	for i := 0; i < numTargets; i++ {
		t := &Target{labels: labels.FromStrings(model.AddressLabel, fmt.Sprintf("example.com:%d", i))}
		l := &testLoop{}
		l.stopFunc = func() {
...
func TestScrapePoolRaces(t *testing.T) {
	...
	tgts := []*targetgroup.Group{{Targets: []model.LabelSet{{model.AddressLabel: "127.0.0.1:9090"}, {model.AddressLabel: "127.0.0.2:9090"}, {model.AddressLabel: "127.0.0.3:9090"}, {model.AddressLabel: "127.0.0.4:9090"}, {model.AddressLabel: "127.0.0.5:9090"}, {model.AddressLabel: "127.0.0.6:9090"}, {model.AddressLabel: "127.0.0.7:9090"}, {model.AddressLabel: "127.0.0.8:9090"}}}}
	sp.Sync(tgts)
...
func TestScrapeLoopStopBeforeRun(t *testing.T) {
	...
	defer _logClusterCodePath()
	scraper := &testScraper{}
	sl := newScrapeLoop(context.Background(), scraper, nil, nil, nopMutator, nopMutator, nil, nil)
...
func TestScrapeLoopStop(t *testing.T) {
	...
	defer close(signal)
	sl := newScrapeLoop(context.Background(), scraper, nil, nil, nopMutator, nopMutator, app, nil)
	numScrapes := 0
...
func TestScrapeLoopRun(t *testing.T) {
	...
	sl := newScrapeLoop(ctx, scraper, nil, nil, nopMutator, nopMutator, app, nil)
...
func TestScrapeLoopMetadata(t *testing.T) {
	...
	defer close(signal)
	ctx, cancel := context.WithCancel(context.Background())
	sl := newScrapeLoop(ctx, scraper, nil, nil, nopMutator, nopMutator, func() storage.Appender {
		return nopAppender{}
	}, cache)
	defer cancel()
...
func TestScrapeLoopAppend(t *testing.T) {
	...
	tests := []struct {
		...
		expValue	float64
	}{{title: "Label name collision", honorLabels: false, scrapeLabels: `metric{n="1"} 0`, discoveryLabels: []string{"n", "2"}, expLset: labels.FromStrings("__name__", "metric", "exported_n", "1", "n", "2"), expValue: 0}, {title: "Delete Empty labels", honorLabels: false, scrapeLabels: `metric{n=""} 0`, discoveryLabels: nil, expLset: labels.FromStrings("__name__", "metric"), expValue: 0}, {title: "Honor Labels", honorLabels: true, scrapeLabels: `metric{n1="1" n2="2"} 0`, discoveryLabels: []string{"n1", "0"}, expLset: labels.FromStrings("__name__", "metric", "n1", "1", "n2", "2"), expValue: 0}, {title: "Stale - NaN", honorLabels: false, scrapeLabels: `metric NaN`, discoveryLabels: nil, expLset: labels.FromStrings("__name__", "metric"), expValue: float64(value.NormalNaN)}}
	for _, test := range tests {
		app := &collectResultAppender{}
		discoveryLabels := &Target{labels: labels.FromStrings(test.discoveryLabels...)}
		sl := newScrapeLoop(context.Background(), nil, nil, nil, func(l labels.Labels) labels.Labels {
		...
		expected := []sample{{metric: test.expLset, t: timestamp.FromTime(now), v: test.expValue}}
...
func TestScrapeLoopAppendSampleLimit(t *testing.T) {
	...
	want := []sample{{metric: labels.FromStrings(model.MetricNameLabel, "metric_a"), t: timestamp.FromTime(now), v: 1}}
...
func TestScrapeLoop_ChangingMetricString(t *testing.T) {
	...
	want := []sample{{metric: labels.FromStrings("__name__", "metric_a", "a", "1", "b", "1"), t: timestamp.FromTime(now), v: 1}, {metric: labels.FromStrings("__name__", "metric_a", "a", "1", "b", "1"), t: timestamp.FromTime(now.Add(time.Minute)), v: 2}}
...
func TestScrapeLoopAppendStaleness(t *testing.T) {
	...
	app.result[1].v = 42
	want := []sample{{metric: labels.FromStrings(model.MetricNameLabel, "metric_a"), t: timestamp.FromTime(now), v: 1}, {metric: labels.FromStrings(model.MetricNameLabel, "metric_a"), t: timestamp.FromTime(now.Add(time.Second)), v: 42}}
...
func TestScrapeLoopAppendNoStalenessIfTimestamp(t *testing.T) {
	...
	want := []sample{{metric: labels.FromStrings(model.MetricNameLabel, "metric_a"), t: 1000, v: 1}}
	if !reflect.DeepEqual(want, app.result) {
		t.Fatalf("Appended samples not as expected. Wanted: %+v Got: %+v", want, app.result)
	}
}
...
type errorAppender struct{ collectResultAppender }
...
func TestScrapeLoopAppendGracefullyIfAmendOrOutOfOrderOrOutOfBounds(t *testing.T) {
	...
	want := []sample{{metric: labels.FromStrings(model.MetricNameLabel, "normal"), t: timestamp.FromTime(now), v: 1}}
	if !reflect.DeepEqual(want, app.result) {
		t.Fatalf("Appended samples not as expected. Wanted: %+v Got: %+v", want, app.result)
	}
}
func TestScrapeLoopOutOfBoundsTimeError(t *testing.T) {
	...
	app := &collectResultAppender{}
	sl := newScrapeLoop(context.Background(), nil, nil, nil, nopMutator, nopMutator, func() storage.Appender {
		return &timeLimitAppender{Appender: app, maxTime: timestamp.FromTime(time.Now().Add(10 * time.Minute))}
	}, nil)
...
func TestTargetScraperScrapeOK(t *testing.T) {
	...
	ts := &targetScraper{Target: &Target{labels: labels.FromStrings(model.SchemeLabel, serverURL.Scheme, model.AddressLabel, serverURL.Host)}, client: http.DefaultClient, timeout: configTimeout}
...
func TestTargetScrapeScrapeCancel(t *testing.T) {
	...
	ts := &targetScraper{Target: &Target{labels: labels.FromStrings(model.SchemeLabel, serverURL.Scheme, model.AddressLabel, serverURL.Host)}, client: http.DefaultClient}
	ctx, cancel := context.WithCancel(context.Background())
	errc := make(chan error)
	go func() {
		time.Sleep(1 * time.Second)
