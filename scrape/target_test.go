...
func TestTargetOffset(t *testing.T) {
	...
	for i := range offsets {
		target := newTestTarget("example.com:80", 0, labels.FromStrings("label", fmt.Sprintf("%d", i)))
...
func TestTargetURL(t *testing.T) {
	...
	expectedParams := url.Values{"abc": []string{"overwrite", "bar", "baz"}, "cde": []string{"huu"}, "xyz": []string{"hoo"}}
	expectedURL := url.URL{Scheme: "https", Host: "example.com:1234", Path: "/metricz", RawQuery: expectedParams.Encode()}
...
func TestNewHTTPBearerToken(t *testing.T) {
	...
	defer server.Close()
	cfg := config_util.HTTPClientConfig{BearerToken: "1234"}
...
func TestNewHTTPBearerTokenFile(t *testing.T) {
	...
	defer server.Close()
	cfg := config_util.HTTPClientConfig{BearerTokenFile: "testdata/bearertoken.txt"}
	c, err := config_util.NewClientFromConfig(cfg, "test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewHTTPBasicAuth(t *testing.T) {
	...
	defer server.Close()
	cfg := config_util.HTTPClientConfig{BasicAuth: &config_util.BasicAuth{Username: "user", Password: "password123"}}
	c, err := config_util.NewClientFromConfig(cfg, "test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewHTTPCACert(t *testing.T) {
	...
	server.StartTLS()
	defer server.Close()
	cfg := config_util.HTTPClientConfig{TLSConfig: config_util.TLSConfig{CAFile: caCertPath}}
	c, err := config_util.NewClientFromConfig(cfg, "test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewHTTPClientCert(t *testing.T) {
	...
	server.StartTLS()
	defer server.Close()
	cfg := config_util.HTTPClientConfig{TLSConfig: config_util.TLSConfig{CAFile: caCertPath, CertFile: "testdata/client.cer", KeyFile: "testdata/client.key"}}
	c, err := config_util.NewClientFromConfig(cfg, "test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewHTTPWithServerName(t *testing.T) {
	...
	server.StartTLS()
	defer server.Close()
	cfg := config_util.HTTPClientConfig{TLSConfig: config_util.TLSConfig{CAFile: caCertPath, ServerName: "prometheus.rocks"}}
	c, err := config_util.NewClientFromConfig(cfg, "test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewHTTPWithBadServerName(t *testing.T) {
	...
	server.StartTLS()
	defer server.Close()
	cfg := config_util.HTTPClientConfig{TLSConfig: config_util.TLSConfig{CAFile: caCertPath, ServerName: "badname"}}
	c, err := config_util.NewClientFromConfig(cfg, "test")
	if err != nil {
		t.Fatal(err)
	}
