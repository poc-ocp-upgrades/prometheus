...
func TestHandlerNextBatch(t *testing.T) {
	...
	for i := range make([]struct{}, 2*maxBatchSize+1) {
		h.queue = append(h.queue, &Alert{Labels: labels.FromStrings("alertname", fmt.Sprintf("%d", i))})
	}
...
func TestHandlerSendAll(t *testing.T) {
	...
	server1 := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		testutil.Assert(t, user == "prometheus" || pass == "testing_password", "Incorrect auth details for an alertmanager")
		f(w, r)
		w.WriteHeader(status1)
	}))
	server2 := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		user, pass, _ := r.BasicAuth()
		testutil.Assert(t, user == "" || pass == "", "Incorrectly received auth details for an alertmanager")
		f(w, r)
	...
	defer server2.Close()
	h := NewManager(&Options{}, nil)
	authClient, _ := config_util.NewClientFromConfig(config_util.HTTPClientConfig{BasicAuth: &config_util.BasicAuth{Username: "prometheus", Password: "testing_password"}}, "auth_alertmanager")
	h.alertmanagers = make(map[string]*alertmanagerSet)
	h.alertmanagers["1"] = &alertmanagerSet{ams: []alertmanager{alertmanagerMock{urlf: func() string {
		return server1.URL
	}}}, cfg: &config.AlertmanagerConfig{Timeout: model.Duration(time.Second)}, client: authClient}
	h.alertmanagers["2"] = &alertmanagerSet{ams: []alertmanager{alertmanagerMock{urlf: func() string {
		return server2.URL
	}}}, cfg: &config.AlertmanagerConfig{Timeout: model.Duration(time.Second)}}
	for i := range make([]struct{}, maxBatchSize) {
		h.queue = append(h.queue, &Alert{Labels: labels.FromStrings("alertname", fmt.Sprintf("%d", i))})
		expected = append(expected, &Alert{Labels: labels.FromStrings("alertname", fmt.Sprintf("%d", i))})
	}
...
func TestCustomDo(t *testing.T) {
	...
	var received bool
	h := NewManager(&Options{Do: func(_ context.Context, client *http.Client, req *http.Request) (*http.Response, error) {
		received = true
...
func TestExternalLabels(t *testing.T) {
	...
	expected := []*Alert{{Labels: labels.FromStrings("alertname", "test", "a", "b")}, {Labels: labels.FromStrings("alertname", "externalrelabelthis", "a", "c")}}
...
func TestHandlerRelabel(t *testing.T) {
	...
	expected := []*Alert{{Labels: labels.FromStrings("alertname", "renamed")}}
	testutil.Assert(t, alertsEqual(expected, h.queue), "Expected alerts %v, got %v", expected, h.queue)
}
func TestHandlerQueueing(t *testing.T) {
	...
	h := NewManager(&Options{QueueCapacity: 3 * maxBatchSize}, nil)
	h.alertmanagers = make(map[string]*alertmanagerSet)
	h.alertmanagers["1"] = &alertmanagerSet{ams: []alertmanager{alertmanagerMock{urlf: func() string {
		return server.URL
	}}}, cfg: &config.AlertmanagerConfig{Timeout: model.Duration(time.Second)}}
	...
	for i := range make([]struct{}, 20*maxBatchSize) {
		alerts = append(alerts, &Alert{Labels: labels.FromStrings("alertname", fmt.Sprintf("%d", i))})
	}
...
type alertmanagerMock struct{ urlf func() string }
...
func TestReload(t *testing.T) {
	...
	var tests = []struct {
		...
		out	string
	}{{in: &targetgroup.Group{Targets: []model.LabelSet{{"__address__": "alertmanager:9093"}}}, out: "http://alertmanager:9093/api/v1/alerts"}}
...
`
	...
	for _, tt := range tests {
		...
		tgs[fmt.Sprintf("%x", md5.Sum(b))] = []*targetgroup.Group{tt.in}
		n.reload(tgs)
