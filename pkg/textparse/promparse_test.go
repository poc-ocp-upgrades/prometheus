...
func TestPromParseErrors(t *testing.T) {
	...
	cases := []struct {
		input	string
		err	string
	}{{input: "a", err: "expected value after metric, got \"MNAME\""}, {input: "a{b='c'} 1\n", err: "expected label value, got \"INVALID\""}, {input: "a{b=\n", err: "expected label value, got \"INVALID\""}, {input: "a{\xff=\"foo\"} 1\n", err: "expected label name, got \"INVALID\""}, {input: "a{b=\"\xff\"} 1\n", err: "invalid UTF-8 label value"}, {input: "a true\n", err: "strconv.ParseFloat: parsing \"true\": invalid syntax"}, {input: "something_weird{problem=\"", err: "expected label value, got \"INVALID\""}, {input: "empty_label_name{=\"\"} 0", err: "expected label name, got \"EQUAL\""}}
	for i, c := range cases {
...
func TestPromNullByteHandling(t *testing.T) {
	...
	cases := []struct {
		input	string
		err	string
	}{{input: "null_byte_metric{a=\"abc\x00\"} 1", err: ""}, {input: "a{b=\"\x00ss\"} 1\n", err: ""}, {input: "a{b=\"\x00\"} 1\n", err: ""}, {input: "a{b=\"\x00\"} 1\n", err: ""}, {input: "a{b=\x00\"ssss\"} 1\n", err: "expected label value, got \"INVALID\""}, {input: "a{b=\"\x00", err: "expected label value, got \"INVALID\""}, {input: "a{b\x00=\"hiih\"}	1", err: "expected equal, got \"INVALID\""}, {input: "a\x00{b=\"ddd\"} 1", err: "expected value after metric, got \"MNAME\""}}
	for i, c := range cases {
		p := NewPromParser([]byte(c.input))
		var err error
		for err == nil {
		...
		for _, fn := range []string{"promtestdata.txt", "promtestdata.nometa.txt"} {
			...
			b.Run("expfmt-text/"+fn, func(b *testing.B) {
				...
				for i := 0; i < b.N; i += promtestdataSampleCount {
					...
					sdec := expfmt.SampleDecoder{Dec: expfmt.NewDecoder(bytes.NewReader(buf), expfmt.FmtText), Opts: &expfmt.DecodeOptions{Timestamp: model.TimeFromUnixNano(0)}}
					for {
