...
func (m rulesRetrieverMock) AlertingRules() []*rules.AlertingRule {
	...
	rule1 := rules.NewAlertingRule("test_metric3", expr1, time.Second, labels.Labels{}, labels.Labels{}, true, log.NewNopLogger())
	rule2 := rules.NewAlertingRule("test_metric4", expr2, time.Second, labels.Labels{}, labels.Labels{}, true, log.NewNopLogger())
...
func (m rulesRetrieverMock) RuleGroups() []*rules.Group {
	...
	defer storage.Close()
	engineOpts := promql.EngineOpts{Logger: nil, Reg: nil, MaxConcurrent: 10, MaxSamples: 10, Timeout: 100 * time.Second}
	engine := promql.NewEngine(engineOpts)
	opts := &rules.ManagerOptions{QueryFunc: rules.EngineQueryFunc(engine, storage), Appendable: storage, Context: context.Background(), Logger: log.NewNopLogger()}
	var r []rules.Rule
...
var samplePrometheusCfg = config.Config{GlobalConfig: config.GlobalConfig{}, AlertingConfig: config.AlertingConfig{}, RuleFiles: []string{}, ScrapeConfigs: []*config.ScrapeConfig{}, RemoteWriteConfigs: []*config.RemoteWriteConfig{}, RemoteReadConfigs: []*config.RemoteReadConfig{}}
...
func TestEndpoints(t *testing.T) {
	...
	t.Run("local", func(t *testing.T) {
		...
		algr.AlertingRules()
		algr.RuleGroups()
		api := &API{Queryable: suite.Storage(), QueryEngine: suite.QueryEngine(), targetRetriever: testTargetRetriever{}, alertmanagerRetriever: testAlertmanagerRetriever{}, flagsMap: sampleFlagMap, now: func() time.Time {
			return now
		}, config: func() config.Config {
			return samplePrometheusCfg
		}, ready: func(f http.HandlerFunc) http.HandlerFunc {
			return f
		}, rulesRetriever: algr}
	...
	t.Run("remote", func(t *testing.T) {
		...
		al.Set("logfmt")
		promlogConfig := promlog.Config{Level: &al, Format: &af}
		remote := remote.NewStorage(promlog.New(&promlogConfig), func() (int64, error) {
			return 0, nil
		}, 1*time.Second)
		err = remote.ApplyConfig(&config.Config{RemoteReadConfigs: []*config.RemoteReadConfig{{URL: &config_util.URL{URL: u}, RemoteTimeout: model.Duration(1 * time.Second), ReadRecent: true}}})
		if err != nil {
			t.Fatal(err)
		}
		var algr rulesRetrieverMock
		...
		algr.AlertingRules()
		algr.RuleGroups()
		api := &API{Queryable: remote, QueryEngine: suite.QueryEngine(), targetRetriever: testTargetRetriever{}, alertmanagerRetriever: testAlertmanagerRetriever{}, flagsMap: sampleFlagMap, now: func() time.Time {
			return now
		}, config: func() config.Config {
			return samplePrometheusCfg
		}, ready: func(f http.HandlerFunc) http.HandlerFunc {
...
func TestLabelNames(t *testing.T) {
	...
	api := &API{Queryable: suite.Storage()}
...
func testEndpoints(t *testing.T, api *API, testLabelAPI bool) {
	...
	var tests = []test{{endpoint: api.query, query: url.Values{"query": []string{"2"}, "time": []string{"123.4"}}, response: &queryData{ResultType: promql.ValueTypeScalar, Result: promql.Scalar{V: 2, T: timestamp.FromTime(start.Add(123*time.Second + 400*time.Millisecond))}}}, {endpoint: api.query, query: url.Values{"query": []string{"0.333"}, "time": []string{"1970-01-01T00:02:03Z"}}, response: &queryData{ResultType: promql.ValueTypeScalar, Result: promql.Scalar{V: 0.333, T: timestamp.FromTime(start.Add(123 * time.Second))}}}, {endpoint: api.query, query: url.Values{"query": []string{"0.333"}, "time": []string{"1970-01-01T01:02:03+01:00"}}, response: &queryData{ResultType: promql.ValueTypeScalar, Result: promql.Scalar{V: 0.333, T: timestamp.FromTime(start.Add(123 * time.Second))}}}, {endpoint: api.query, query: url.Values{"query": []string{"0.333"}}, response: &queryData{ResultType: promql.ValueTypeScalar, Result: promql.Scalar{V: 0.333, T: timestamp.FromTime(api.now())}}}, {endpoint: api.queryRange, query: url.Values{"query": []string{"time()"}, "start": []string{"0"}, "end": []string{"2"}, "step": []string{"1"}}, response: &queryData{ResultType: promql.ValueTypeMatrix, Result: promql.Matrix{promql.Series{Points: []promql.Point{{V: 0, T: timestamp.FromTime(start)}, {V: 1, T: timestamp.FromTime(start.Add(1 * time.Second))}, {V: 2, T: timestamp.FromTime(start.Add(2 * time.Second))}}, Metric: nil}}}}, {endpoint: api.queryRange, query: url.Values{"query": []string{"time()"}, "end": []string{"2"}, "step": []string{"1"}}, errType: errorBadData}, {endpoint: api.queryRange, query: url.Values{"query": []string{"time()"}, "start": []string{"0"}, "step": []string{"1"}}, errType: errorBadData}, {endpoint: api.queryRange, query: url.Values{"query": []string{"time()"}, "start": []string{"0"}, "end": []string{"2"}}, errType: errorBadData}, {endpoint: api.query, query: url.Values{"query": []string{"invalid][query"}, "time": []string{"1970-01-01T01:02:03+01:00"}}, errType: errorBadData}, {endpoint: api.queryRange, query: url.Values{"query": []string{"invalid][query"}, "start": []string{"0"}, "end": []string{"100"}, "step": []string{"1"}}, errType: errorBadData}, {endpoint: api.queryRange, query: url.Values{"query": []string{"time()"}, "start": []string{"1"}, "end": []string{"2"}, "step": []string{"0"}}, errType: errorBadData}, {endpoint: api.queryRange, query: url.Values{"query": []string{"time()"}, "start": []string{"2"}, "end": []string{"1"}, "step": []string{"1"}}, errType: errorBadData}, {endpoint: api.queryRange, query: url.Values{"query": []string{"time()"}, "start": []string{"148966367200.372"}, "end": []string{"1489667272.372"}, "step": []string{"1"}}, errType: errorBadData}, {endpoint: api.series, query: url.Values{"match[]": []string{`test_metric2`}}, response: []labels.Labels{labels.FromStrings("__name__", "test_metric2", "foo", "boo")}}, {endpoint: api.series, query: url.Values{"match[]": []string{`test_metric1{foo=~".+o"}`}}, response: []labels.Labels{labels.FromStrings("__name__", "test_metric1", "foo", "boo")}}, {endpoint: api.series, query: url.Values{"match[]": []string{`test_metric1{foo=~".+o$"}`, `test_metric1{foo=~".+o"}`}}, response: []labels.Labels{labels.FromStrings("__name__", "test_metric1", "foo", "boo")}}, {endpoint: api.series, query: url.Values{"match[]": []string{`test_metric1{foo=~".+o"}`, `none`}}, response: []labels.Labels{labels.FromStrings("__name__", "test_metric1", "foo", "boo")}}, {endpoint: api.series, query: url.Values{"match[]": []string{`test_metric2`}, "start": []string{"-2"}, "end": []string{"-1"}}, response: []labels.Labels{}}, {endpoint: api.series, query: url.Values{"match[]": []string{`test_metric2`}, "start": []string{"100000"}, "end": []string{"100001"}}, response: []labels.Labels{}}, {endpoint: api.series, query: url.Values{"match[]": []string{`test_metric2`}, "start": []string{"-1"}, "end": []string{"100000"}}, response: []labels.Labels{labels.FromStrings("__name__", "test_metric2", "foo", "boo")}}, {endpoint: api.series, query: url.Values{"match[]": []string{`test_metric2`}, "start": []string{"1"}, "end": []string{"100"}}, response: []labels.Labels{labels.FromStrings("__name__", "test_metric2", "foo", "boo")}}, {endpoint: api.series, query: url.Values{"match[]": []string{`test_metric2`}, "start": []string{"1"}, "end": []string{"100000"}}, response: []labels.Labels{labels.FromStrings("__name__", "test_metric2", "foo", "boo")}}, {endpoint: api.series, query: url.Values{"match[]": []string{`test_metric2`}, "start": []string{"-1"}, "end": []string{"1"}}, response: []labels.Labels{labels.FromStrings("__name__", "test_metric2", "foo", "boo")}}, {endpoint: api.series, errType: errorBadData}, {endpoint: api.dropSeries, errType: errorInternal}, {endpoint: api.targets, response: &TargetDiscovery{ActiveTargets: []*Target{{DiscoveredLabels: map[string]string{}, Labels: map[string]string{"job": "blackbox"}, ScrapeURL: "http://localhost:9115/probe?target=example.com", Health: "unknown"}, {DiscoveredLabels: map[string]string{}, Labels: map[string]string{"job": "test"}, ScrapeURL: "http://example.com:8080/metrics", Health: "unknown"}}, DroppedTargets: []*DroppedTarget{{DiscoveredLabels: map[string]string{"__address__": "http://dropped.example.com:9115", "__metrics_path__": "/probe", "__scheme__": "http", "job": "blackbox"}}}}}, {endpoint: api.alertmanagers, response: &AlertmanagerDiscovery{ActiveAlertmanagers: []*AlertmanagerTarget{{URL: "http://alertmanager.example.com:8080/api/v1/alerts"}}, DroppedAlertmanagers: []*AlertmanagerTarget{{URL: "http://dropped.alertmanager.example.com:8080/api/v1/alerts"}}}}, {endpoint: api.serveConfig, response: &prometheusConfig{YAML: samplePrometheusCfg.String()}}, {endpoint: api.serveFlags, response: sampleFlagMap}, {endpoint: api.alerts, response: &AlertDiscovery{Alerts: []*Alert{}}}, {endpoint: api.rules, response: &RuleDiscovery{RuleGroups: []*RuleGroup{{Name: "grp", File: "/path/to/file", Interval: 1, Rules: []rule{alertingRule{Name: "test_metric3", Query: "absent(test_metric3) != 1", Duration: 1, Labels: labels.Labels{}, Annotations: labels.Labels{}, Alerts: []*Alert{}, Health: "unknown", Type: "alerting"}, alertingRule{Name: "test_metric4", Query: "up == 1", Duration: 1, Labels: labels.Labels{}, Annotations: labels.Labels{}, Alerts: []*Alert{}, Health: "unknown", Type: "alerting"}, recordingRule{Name: "recording-rule-1", Query: "vector(1)", Labels: labels.Labels{}, Health: "unknown", Type: "recording"}}}}}}}
	if testLabelAPI {
		tests = append(tests, []test{{endpoint: api.labelValues, params: map[string]string{"name": "__name__"}, response: []string{"test_metric1", "test_metric2"}}, {endpoint: api.labelValues, params: map[string]string{"name": "foo"}, response: []string{"bar", "boo"}}, {endpoint: api.labelValues, params: map[string]string{"name": "not!!!allowed"}, errType: errorBadData}, {endpoint: api.labelNames, response: []string{"__name__", "foo"}}}...)
	}
...
func TestReadEndpoint(t *testing.T) {
	...
	api := &API{Queryable: suite.Storage(), QueryEngine: suite.QueryEngine(), config: func() config.Config {
		return config.Config{GlobalConfig: config.GlobalConfig{ExternalLabels: model.LabelSet{"baz": "a", "b": "c", "d": "e"}}}
	}, remoteReadSampleLimit: 1e6, remoteReadGate: gate.New(1)}
	...
	expected := &prompb.QueryResult{Timeseries: []*prompb.TimeSeries{{Labels: []prompb.Label{{Name: "__name__", Value: "test_metric1"}, {Name: "b", Value: "c"}, {Name: "baz", Value: "qux"}, {Name: "d", Value: "e"}, {Name: "foo", Value: "bar"}}, Samples: []prompb.Sample{{Value: 1, Timestamp: 0}}}}}
...
func TestAdminEndpoints(t *testing.T) {
	...
	snapshotAPI := func(api *API) apiFunc {
		return api.snapshot
	}
	cleanAPI := func(api *API) apiFunc {
		return api.cleanTombstones
	}
	deleteAPI := func(api *API) apiFunc {
		return api.deleteSeries
	}
	for i, tc := range []struct {
		...
		values		url.Values
		errType		errorType
	}{{db: tsdb, enableAdmin: false, endpoint: snapshotAPI, errType: errorUnavailable}, {db: tsdb, enableAdmin: true, endpoint: snapshotAPI, errType: errorNone}, {db: tsdb, enableAdmin: true, endpoint: snapshotAPI, values: map[string][]string{"skip_head": {"true"}}, errType: errorNone}, {db: tsdb, enableAdmin: true, endpoint: snapshotAPI, values: map[string][]string{"skip_head": {"xxx"}}, errType: errorBadData}, {db: tsdbWithError, enableAdmin: true, endpoint: snapshotAPI, errType: errorInternal}, {db: nil, enableAdmin: true, endpoint: snapshotAPI, errType: errorUnavailable}, {db: tsdb, enableAdmin: false, endpoint: cleanAPI, errType: errorUnavailable}, {db: tsdb, enableAdmin: true, endpoint: cleanAPI, errType: errorNone}, {db: tsdbWithError, enableAdmin: true, endpoint: cleanAPI, errType: errorInternal}, {db: nil, enableAdmin: true, endpoint: cleanAPI, errType: errorUnavailable}, {db: tsdb, enableAdmin: false, endpoint: deleteAPI, errType: errorUnavailable}, {db: tsdb, enableAdmin: true, endpoint: deleteAPI, errType: errorBadData}, {db: tsdb, enableAdmin: true, endpoint: deleteAPI, values: map[string][]string{"match[]": {"123"}}, errType: errorBadData}, {db: tsdb, enableAdmin: true, endpoint: deleteAPI, values: map[string][]string{"match[]": {"up"}, "start": {"xxx"}}, errType: errorBadData}, {db: tsdb, enableAdmin: true, endpoint: deleteAPI, values: map[string][]string{"match[]": {"up"}, "end": {"xxx"}}, errType: errorBadData}, {db: tsdb, enableAdmin: true, endpoint: deleteAPI, values: map[string][]string{"match[]": {"up"}}, errType: errorNone}, {db: tsdb, enableAdmin: true, endpoint: deleteAPI, values: map[string][]string{"match[]": {"up{job!=\"foo\"}", "{job=~\"bar.+\"}", "up{instance!~\"fred.+\"}"}}, errType: errorNone}, {db: tsdbWithError, enableAdmin: true, endpoint: deleteAPI, values: map[string][]string{"match[]": {"up"}}, errType: errorInternal}, {db: nil, enableAdmin: true, endpoint: deleteAPI, errType: errorUnavailable}} {
		tc := tc
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			api := &API{db: func() TSDBAdmin {
				if tc.db != nil {
					return tc.db
				}
				return nil
...
func TestRespondSuccess(t *testing.T) {
	...
	exp := &response{Status: statusSuccess, Data: "test"}
...
func TestRespondError(t *testing.T) {
	...
	exp := &response{Status: statusError, Data: "test", ErrorType: errorTimeout, Error: "message"}
	if !reflect.DeepEqual(&res, exp) {
		t.Fatalf("Expected response \n%v\n but got \n%v\n", res, exp)
	}
}
func TestParseTime(t *testing.T) {
	...
	var tests = []struct {
		...
		fail	bool
		result	time.Time
	}{{input: "", fail: true}, {input: "abc", fail: true}, {input: "30s", fail: true}, {input: "123", result: time.Unix(123, 0)}, {input: "123.123", result: time.Unix(123, 123000000)}, {input: "2015-06-03T13:21:58.555Z", result: ts}, {input: "2015-06-03T14:21:58.555+01:00", result: ts}, {input: "1543578564.705", result: time.Unix(1543578564, 705*1e6)}}
...
func TestParseDuration(t *testing.T) {
	...
	}{{input: "", fail: true}, {input: "abc", fail: true}, {input: "2015-06-03T13:21:58.555Z", fail: true}, {input: "-148966367200.372", fail: true}, {input: "148966367200.372", fail: true}, {input: "123", result: 123 * time.Second}, {input: "123.333", result: 123*time.Second + 333*time.Millisecond}, {input: "15s", result: 15 * time.Second}, {input: "5m", result: 5 * time.Minute}}
	for _, test := range tests {
...
func BenchmarkRespond(b *testing.B) {
	...
	response := &queryData{ResultType: promql.ValueTypeMatrix, Result: promql.Matrix{promql.Series{Points: points, Metric: nil}}}
	b.ResetTimer()
	api := API{}
	for n := 0; n < b.N; n++ {
