...
func TestAlertingRule(t *testing.T) {
	...
	testutil.Ok(t, err)
	rule := NewAlertingRule("HTTPRequestRateLow", expr, time.Minute, labels.FromStrings("severity", "{{\"c\"}}ritical"), nil, true, nil)
	result := promql.Vector{{Metric: labels.FromStrings("__name__", "ALERTS", "alertname", "HTTPRequestRateLow", "alertstate", "pending", "group", "canary", "instance", "0", "job", "app-server", "severity", "critical"), Point: promql.Point{V: 1}}, {Metric: labels.FromStrings("__name__", "ALERTS", "alertname", "HTTPRequestRateLow", "alertstate", "pending", "group", "canary", "instance", "1", "job", "app-server", "severity", "critical"), Point: promql.Point{V: 1}}, {Metric: labels.FromStrings("__name__", "ALERTS", "alertname", "HTTPRequestRateLow", "alertstate", "firing", "group", "canary", "instance", "0", "job", "app-server", "severity", "critical"), Point: promql.Point{V: 1}}, {Metric: labels.FromStrings("__name__", "ALERTS", "alertname", "HTTPRequestRateLow", "alertstate", "firing", "group", "canary", "instance", "1", "job", "app-server", "severity", "critical"), Point: promql.Point{V: 1}}}
	baseTime := time.Unix(0, 0)
	var tests = []struct {
		time	time.Duration
		result	promql.Vector
	}{{time: 0, result: result[:2]}, {time: 5 * time.Minute, result: result[2:]}, {time: 10 * time.Minute, result: result[2:3]}, {time: 15 * time.Minute, result: nil}, {time: 20 * time.Minute, result: nil}, {time: 25 * time.Minute, result: result[:1]}, {time: 30 * time.Minute, result: result[2:3]}}
...
func TestForStateAddSamples(t *testing.T) {
	...
	testutil.Ok(t, err)
	rule := NewAlertingRule("HTTPRequestRateLow", expr, time.Minute, labels.FromStrings("severity", "{{\"c\"}}ritical"), nil, true, nil)
	result := promql.Vector{{Metric: labels.FromStrings("__name__", "ALERTS_FOR_STATE", "alertname", "HTTPRequestRateLow", "group", "canary", "instance", "0", "job", "app-server", "severity", "critical"), Point: promql.Point{V: 1}}, {Metric: labels.FromStrings("__name__", "ALERTS_FOR_STATE", "alertname", "HTTPRequestRateLow", "group", "canary", "instance", "1", "job", "app-server", "severity", "critical"), Point: promql.Point{V: 1}}, {Metric: labels.FromStrings("__name__", "ALERTS_FOR_STATE", "alertname", "HTTPRequestRateLow", "group", "canary", "instance", "0", "job", "app-server", "severity", "critical"), Point: promql.Point{V: 1}}, {Metric: labels.FromStrings("__name__", "ALERTS_FOR_STATE", "alertname", "HTTPRequestRateLow", "group", "canary", "instance", "1", "job", "app-server", "severity", "critical"), Point: promql.Point{V: 1}}}
	baseTime := time.Unix(0, 0)
	var tests = []struct {
		time		time.Duration
		result		promql.Vector
		persistThisTime	bool
	}{{time: 0, result: append(promql.Vector{}, result[:2]...), persistThisTime: true}, {time: 5 * time.Minute, result: append(promql.Vector{}, result[2:]...)}, {time: 10 * time.Minute, result: append(promql.Vector{}, result[2:3]...)}, {time: 15 * time.Minute, result: nil}, {time: 20 * time.Minute, result: nil}, {time: 25 * time.Minute, result: append(promql.Vector{}, result[:1]...), persistThisTime: true}, {time: 30 * time.Minute, result: append(promql.Vector{}, result[2:3]...)}}
...
func TestForStateRestore(t *testing.T) {
	...
	expr, err := promql.ParseExpr(`http_requests{group="canary", job="app-server"} < 100`)
	testutil.Ok(t, err)
	opts := &ManagerOptions{QueryFunc: EngineQueryFunc(suite.QueryEngine(), suite.Storage()), Appendable: suite.Storage(), TSDB: suite.Storage(), Context: context.Background(), Logger: log.NewNopLogger(), NotifyFunc: func(ctx context.Context, expr string, alerts ...*Alert) {
	}, OutageTolerance: 30 * time.Minute, ForGracePeriod: 10 * time.Minute}
	alertForDuration := 25 * time.Minute
	rule := NewAlertingRule("HTTPRequestRateLow", expr, alertForDuration, labels.FromStrings("severity", "critical"), nil, true, nil)
	...
	tests := []testInput{{restoreDuration: 15 * time.Minute, alerts: rule.ActiveAlerts(), downDuration: 10 * time.Minute}, {restoreDuration: 40 * time.Minute, noRestore: true, num: 2}, {restoreDuration: 50 * time.Minute, alerts: []*Alert{}}}
	testFunc := func(tst testInput) {
		newRule := NewAlertingRule("HTTPRequestRateLow", expr, alertForDuration, labels.FromStrings("severity", "critical"), nil, false, nil)
	...
	testFunc(testInput{restoreDuration: 25 * time.Minute, alerts: []*Alert{}, gracePeriod: true, num: 2})
}
func TestStaleness(t *testing.T) {
	...
	opts := &ManagerOptions{QueryFunc: EngineQueryFunc(engine, storage), Appendable: storage, TSDB: storage, Context: context.Background(), Logger: log.NewNopLogger()}
	...
	metricSample[2].V = 42
	want := map[string][]promql.Point{metric: {{0, 2}, {1000, 3}, {2000, 42}}}
...
func TestCopyState(t *testing.T) {
	...
	want := []map[string]labels.Labels{{"r3a": nil}, {"r3b": nil}, nil, {"a": nil}, {"r1": nil}, nil}
...
func TestUpdate(t *testing.T) {
	...
	expected := map[string]labels.Labels{"test": labels.FromStrings("name", "value")}
	storage := testutil.NewStorage(t)
	defer storage.Close()
	opts := promql.EngineOpts{Logger: nil, Reg: nil, MaxConcurrent: 10, MaxSamples: 10, Timeout: 10 * time.Second}
	engine := promql.NewEngine(opts)
	ruleManager := NewManager(&ManagerOptions{Appendable: storage, TSDB: storage, QueryFunc: EngineQueryFunc(engine, storage), Context: context.Background(), Logger: log.NewNopLogger()})
	ruleManager.Run()
	...
	for _, g := range ruleManager.groups {
		g.seriesInPreviousEval = []map[string]labels.Labels{expected}
	}
	err = ruleManager.Update(10*time.Second, files)
	testutil.Ok(t, err)
	for _, g := range ruleManager.groups {
...
func TestNotify(t *testing.T) {
	...
	opts := &ManagerOptions{QueryFunc: EngineQueryFunc(engine, storage), Appendable: storage, TSDB: storage, Context: context.Background(), Logger: log.NewNopLogger(), NotifyFunc: notifyFunc, ResendDelay: 2 * time.Second}
	expr, err := promql.ParseExpr("a > 1")
	testutil.Ok(t, err)
